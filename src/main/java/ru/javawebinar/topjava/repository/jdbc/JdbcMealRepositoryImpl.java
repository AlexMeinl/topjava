...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    @Autowired
    public JdbcMealRepositoryImpl(JdbcTemplate jdbcTemplate, DataSource dataSource, NamedParameterJdbcTemplate namedParameterJdbcTemplate)
    {
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=?", ROW_MAPPER, userId)
                .stream()
                .sorted((m1, m2) -> m2.getDateTime().toString().compareTo(m1.getDateTime().toString()))
